...
def gumbel_sample(
    ...
    dtype, size = distance.dtype, distance.shape[dim]
    if stochastic:
        '''
        pr = (logits/ temperature).softmax(dim = -1) # ref: https://bobondemon.github.io/2021/08/07/Gumbel-Max-Trick/  torch.Size([1, 24064, 2048])    
        ind = torch.zeros(1, pr.shape[1], dtype=int, device='cuda')
        for f in range(pr.shape[1]):
            ind[0,f] = np.random.choice(pr.shape[-1], size=1, p=pr[0,f,:].cpu().numpy())[0]
        '''
        pr = (distance/ temperature).softmax(dim = -1) # ref: https://bobondemon.github.io/2021/08/07/Gumbel-Max-Trick/  torch.Size([1, 24064, 2048])
        logits = torch.log(pr)
        sampling_logits = logits + gumbel_noise(logits)
        #sampling_logits = (logits / temperature) + gumbel_noise(logits)
    else:
        sampling_logits = distance
    ind = sampling_logits.argmax(dim = dim) # torch.Size([1, 24064])
...
class EuclideanCodebook(nn.Module):
    ...
    def forward(
        ...
        embed_ind, embed_onehot = self.gumbel_sample(dist, dim = -1, temperature = sample_codebook_temp, training = self.training, stochastic=stochastic)
        ...
        if self.training and self.ema_update and update:
